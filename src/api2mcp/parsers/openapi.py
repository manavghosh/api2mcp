...
class RefResolver:
    ...
    async def resolve(
        ...
        if doc_part:
            doc = await self._load_external(doc_part)
        else:
            doc = self._root
        ...
        if isinstance(target, dict) and "$ref" in target:
            return await self.resolve(target["$ref"], visited)

        return target

    _MAX_RESOLVE_DEPTH: int = 50  # T10: guard against deeply-nested / "billion laughs" specs

    async def resolve_all_refs(
        self,
        obj: Any,
        ...
        Args:
            ...
            _depth: Current recursion depth (internal — raises after _MAX_RESOLVE_DEPTH).

        Returns:
            The fully resolved object tree.

        Raises:
            RefResolutionError: If recursion exceeds _MAX_RESOLVE_DEPTH (T10 DoS protection).
        """
        if _depth > self._MAX_RESOLVE_DEPTH:
            raise RefResolutionError(
                str(obj)[:80],
                f"$ref resolution exceeded maximum depth ({self._MAX_RESOLVE_DEPTH}) "
                "— possible recursive or deeply-nested spec (T10 DoS protection)",
            )
        if visited is None:
            visited = set()
        ...
        if isinstance(obj, dict):
            if "$ref" in obj:
                ...
                try:
                    resolved = await self.resolve(ref, visited)
                except CircularRefError:
                    logger.warning("Circular $ref skipped: %s", ref)
                    return {"_circular_ref": ref}
                return await self.resolve_all_refs(resolved, visited | {ref}, _depth + 1)
            result = {}
            for key, value in obj.items():
                result[key] = await self.resolve_all_refs(value, visited, _depth + 1)
            return result
        if isinstance(obj, list):
            return [await self.resolve_all_refs(item, visited, _depth + 1) for item in obj]
        return obj

    async def _load_external(self, doc_path: str) -> dict[str, Any]:
        """Load an external document (file or URL) with double-checked async locking."""
        # First check (without lock) for fast path
        if doc_path in self._external_cache:
            return self._external_cache[doc_path]

        async with self._cache_lock:
            # Second check (under lock) to avoid redundant loads
            if doc_path in self._external_cache:
                return self._external_cache[doc_path]

            parsed_url = urlparse(doc_path)
            if parsed_url.scheme in ("http", "https"):
                doc = await self._load_url(doc_path)
    ...
    def _check_ssrf(url: str) -> None:
        """Raise RefResolutionError if *url* resolves to a private/reserved address (T02).

        Blocks access to cloud metadata services (169.254.169.254), loopback, and
        RFC-1918 private ranges to prevent Server-Side Request Forgery via $ref.
        """
        parsed = urlparse(url)
        host = parsed.hostname or ""
        if not host:
            raise RefResolutionError(url, "Remote $ref has no resolvable hostname")
        try:
            # Resolve DNS to IP; use the first result
            ...
            for _, _, _, _, sockaddr in addr_info:
                ...
                if ip.is_private or ip.is_loopback or ip.is_link_local or ip.is_reserved:
                    raise RefResolutionError(
                        url,
                        f"Remote $ref blocked: {host!r} resolves to a private/reserved "
                        f"address ({ip}) — SSRF protection (T02)",
                    )
        except socket.gaierror as exc:
            raise RefResolutionError(url, f"Cannot resolve hostname for remote $ref: {exc}") from exc

    async def _load_url(self, url: str) -> dict[str, Any]:
...
async def _schema_to_ir(
    ...
    if "properties" in raw:
        ...
        for name, prop_schema in raw["properties"].items():
            props[name] = await _schema_to_ir(prop_schema, resolver)
        sr.properties = props

    # Array items
    if "items" in raw:
        sr.items = await _schema_to_ir(raw["items"], resolver)

    # additionalProperties
    ap = raw.get("additionalProperties")
    if isinstance(ap, dict):
        sr.additional_properties = await _schema_to_ir(ap, resolver)
    elif isinstance(ap, bool):
        sr.additional_properties = ap

    # Composition
    if "oneOf" in raw:
        sr.one_of = [await _schema_to_ir(s, resolver) for s in raw["oneOf"]]
    if "anyOf" in raw:
        sr.any_of = [await _schema_to_ir(s, resolver) for s in raw["anyOf"]]
    if "allOf" in raw:
        sr.all_of = [await _schema_to_ir(s, resolver) for s in raw["allOf"]]
...
async def _extract_auth_schemes(
