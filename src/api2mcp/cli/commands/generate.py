...
def generate_cmd(
    ...
    resolved_output = Path(effective.get("output") or "generated").resolve()  # T12: canonicalize
...
def _write_server_module(
    ...
    *,
    ...
    # T01: strip triple-quote sequences that would break the f-string template or
    # the generated module docstring. Embedded values are repr()-quoted for
    # _BASE_URL / _SERVER_NAME / _SERVER_VERSION (already safe); the docstring
    # line uses plain string interpolation so we sanitize it separately.
    safe_name = name.replace("'''", "").replace('"""', "").replace("\n", " ").strip()
    safe_version = version.replace("'''", "").replace('"""', "").replace("\n", " ").strip()

    server_code = f'''\
"""Generated MCP server for {safe_name} v{safe_version}.
